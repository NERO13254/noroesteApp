...
const { updateWaferExported } = require("../functtions/printCommitmentSheet/updateWaferExported");
const {backBtn} = require("../functtions/backBtn");
const { evaluateWafer } = require("../functtions/printCommitmentSheet/evaluateWafer");
...
let waferExist          = 0;
let dataCilinder = [];
// se utiliza para guardar los cilindros por defecto de la oblea guardada
var waferCilinderDataObjContent = {}

// se usa para guardar las valvulas por defecto de la oblea guardada
var waferValveDataSaved = {}
...
window.onload = async()=>{
    ...
    backBtn(document.getElementById("backBtn", "seeSavedCilinders"))
    ...
    // carga los datos por defecto del  cilindor
    ...
    goToPrint.addEventListener( "click", async()=>{
        ...
        if(document.getElementById("wafer").value.length<=0 || document.getElementById("wafer").value=="NO"){
            ...
            // evalua que los datos de la oblea sean correctos
            let evaluateWaferSucces = await evaluateWafer();
         
            if(evaluateWaferSucces){
                await collectData.collectData();
                // funcion que guarda o actualiza la oblea en la tabla wafersdata
                await saveWaferInDb();
                                
                // evalua si existe la oblea en obleas generales y actualiza los datos relacionados
                await updateWaferExported();
    ...
    if(document.getElementById("exportToEnergas")){
